...
void EditorScene::DrawAnimatorInspector(WorldEntity* entity) {
    if (!entity->animator || DrawAnimatorHeader(entity)) {
        return;
    }
    AnimatorComponent& animator = *entity->animator;
    DrawAnimatorRuntimeStatus(animator);
    const EntityId selectionBefore = selection_;
    std::string before = WorldSerializer::Serialize(world_);
    ...
    const Model* model = ResolveAnimatorInspectorModel(*entity);
    DrawAnimatorClipSelection(*entity, animator, model, selectionBefore);
    DrawAnimatorPreviewControls(entity->id, model);
    DrawAnimatorPlaybackSettings(*entity, animator, selectionBefore);
}
