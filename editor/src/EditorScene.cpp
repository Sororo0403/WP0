...
bool AddAssetImportFile(std::vector<AssetImportFile>& files,
    ...
    const auto existing = std::ranges::find_if(files, [&normalized](const AssetImportFile& item) {
...
bool CollectGltfDependencies(const std::filesystem::path& gltfPath,
                             std::vector<AssetImportFile>& files,
                             std::string& errorMessage) {
...
bool FilesHaveEqualContents(const std::filesystem::path& left,
...
bool AddLocalObjDependency(const std::filesystem::path& objRoot,
    ...
    if (relative.empty() || relative.is_absolute() || relative.has_root_name() ||
        ...
        !IsPathWithinRoot(objRoot, source)) {
...
bool EditorScene::ImportAssetFiles() {
    ...
    std::vector<AssetImportFile> importFiles;
    ...
    for (const std::filesystem::path& source : selectedFiles) {
        ...
        if (!AddAssetImportFile(importFiles, source, source.filename(), importError)) {
        ...
        if (extension == ".gltf" &&
            !CollectGltfDependencies(source, importFiles, importError)) {
            status_ = "Asset import stopped: " + importError;
            return false;
        }
        if (extension == ".obj" &&
            !CollectObjDependencies(source, importFiles, importError)) {
            status_ = "Asset import stopped: " + importError;
            return false;
        }
    }
    ...
    std::error_code error;
    for (const AssetImportFile& file : importFiles) {
        ...
        if (error || !std::filesystem::is_regular_file(destination, error) || error ||
            !FilesHaveEqualContents(file.source, destination)) {
