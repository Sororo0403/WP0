...
void EditorScene::ResetGameUiEditingState() {
    gameUiDragEntity_ = {};
    gameUiResizeEntity_ = {};
    gameUiResizeHandle_ = UiResizeHandle::None;
}

EntityId EditorScene::FindHoveredGameUiEntity(const ImVec2& mouse, const ImVec2& imageMin,
...
EditorScene::UiResizeHandle EditorScene::FindHoveredGameUiResizeHandle(
    const ImVec2& mouse, const ImVec2& imageMin, const ImVec2& imageMax) const {
    const WorldEntity* selected = world_.Find(selection_);
    ImVec2 selectedImageMin{};
    ImVec2 selectedImageMax{};
    if (selected == nullptr ||
        !TryCalculateGameUiImageRect(
            *selected, imageMin, imageMax, selectedImageMin, selectedImageMax)) {
    ...
    if (hitHandle({selectedImageMax.x, selectedImageMin.y})) {
        return UiResizeHandle::TopRight;
    }
    if (hitHandle({selectedImageMin.x, selectedImageMax.y})) {
        return UiResizeHandle::BottomLeft;
    }
    return hitHandle(selectedImageMax) ? UiResizeHandle::BottomRight : UiResizeHandle::None;
}

void EditorScene::HandleGameUiPointerInput(bool imageHovered, EntityId hovered,
    ...
    if (hoveredResizeHandle != UiResizeHandle::None) {
        ...
        ResetGameUiEditingState();
    }
}

void EditorScene::HandleGameUiKeyboardNudge(const ImVec2& imageMin,
    ...
    nudge.x -= ImGui::IsKeyPressed(ImGuiKey_LeftArrow, true) ? distance : 0.0f;
    nudge.x += ImGui::IsKeyPressed(ImGuiKey_RightArrow, true) ? distance : 0.0f;
    nudge.y -= ImGui::IsKeyPressed(ImGuiKey_UpArrow, true) ? distance : 0.0f;
    nudge.y += ImGui::IsKeyPressed(ImGuiKey_DownArrow, true) ? distance : 0.0f;
    ...
    if ((nudge.x == 0.0f && nudge.y == 0.0f) || entity == nullptr ||
        !TryCalculateGameUiRect(
            *entity, imageMin, imageMax, selectedRectMin, selectedRectMax)) {
        return;
    }
    ...
    status_ = io.KeyShift ? "Moved UI element by 10 pixels."
                          : "Moved UI element by 1 pixel.";
}

void EditorScene::DrawGameUiSelectionOverlay(const ImVec2& imageMin,
                                              const ImVec2& imageMax) const {
    const WorldEntity* selected = world_.Find(selection_);
    ImVec2 selectedMin{};
    ImVec2 selectedMax{};
    ...
    if (TryCalculateGameUiImageRect(
            *selected, imageMin, imageMax, imageRectMin, imageRectMax)) {
...
void EditorScene::UpdateGameUiEditingCursor(
    EntityId hovered, UiResizeHandle hoveredResizeHandle) const {
...
void EditorScene::HandleGameUiEditing(const ImVec2& imageMin, const ImVec2& imageMax) {
    ...
    const EntityId hovered =
        imageHovered ? FindHoveredGameUiEntity(mouse, imageMin, imageMax) : EntityId{};
    const UiResizeHandle hoveredResizeHandle =
        imageHovered ? FindHoveredGameUiResizeHandle(mouse, imageMin, imageMax)
                     : UiResizeHandle::None;
    HandleGameUiPointerInput(imageHovered, hovered, hoveredResizeHandle);
    UpdateGameUiDragAndResize(imageMin, imageMax);
    HandleGameUiKeyboardNudge(imageMin, imageMax);
    DrawGameUiSelectionOverlay(imageMin, imageMax);
    UpdateGameUiEditingCursor(hovered, hoveredResizeHandle);
}
