...
bool EditorScene::DrawTextRemoval(WorldEntity* entity) {
...
void EditorScene::DrawTextGeneralSettings(TextComponent& text, EntityId selectionBefore) {
...
void EditorScene::DrawTextContentAndPosition(TextComponent& text) {
    std::array<char, 4097> textBuffer{};
    std::memcpy(textBuffer.data(), text.text.data(),
                (std::min)(text.text.size(), textBuffer.size() - 1u));
    if (ImGui::InputTextMultiline(
            "Content##Text", textBuffer.data(), textBuffer.size(), {-FLT_MIN, 80.0f})) {
...
void EditorScene::DrawTextFontPicker(TextComponent& text, EntityId selectionBefore) {
    ...
    if (ImGui::BeginDragDropTarget()) {
        if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(kFontAssetDragPayload);
    ...
    if (ImGui::BeginPopup("TextFontPicker")) {
        ...
        for (const std::filesystem::path& fontAsset : fontAssets_) {
            ...
            const std::string label =
                fontAsset.filename().generic_string() + "##TextFont" + fontAsset.generic_string();
...
void EditorScene::DrawTextFontStatus(const TextComponent& text) {
...
void EditorScene::DrawTextTypographySettings(TextComponent& text) {
    ...
    if (ImGui::DragFloat("Line Spacing##Text", &text.lineSpacing, 0.5f, 0.0f, 512.0f, "%.1f",
                         ImGuiSliderFlags_AlwaysClamp)) {
        RefreshDirty();
...
void EditorScene::DrawTextColorAndAlignment(TextComponent& text,
    ...
    const auto selectAlignment = [&](TextAlignment value, const char* label) {
        if (ImGui::Selectable(label, text.alignment == value)) {
            ...
            RecordImmediateEdit("Modify Text Alignment", alignmentBefore, selectionBefore);
...
void EditorScene::DrawTextCanvasRequirement(const WorldEntity* entity) {
...
void EditorScene::DrawTextInspector(WorldEntity* entity) {
    ...
    if (DrawTextRemoval(entity)) {
        return;
    }
    TextComponent& text = *entity->text;
    const EntityId selectionBefore = selection_;
    DrawTextGeneralSettings(text, selectionBefore);
    DrawTextContentAndPosition(text);
    DrawTextFontPicker(text, selectionBefore);
    DrawTextFontStatus(text);
    DrawTextTypographySettings(text);
    DrawTextColorAndAlignment(text, selectionBefore);
    DrawTextCanvasRequirement(entity);
}
