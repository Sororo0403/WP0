...
bool EditorScene::DrawAssetScriptPropertyInspector(WorldEntity* entity, BehaviorComponent& behavior,
                                                   const ScriptPropertyDefinition& definition,
                                                   EntityId selectionBefore) {
    if (definition.type != ScriptPropertyType::AnimationClip &&
        definition.type != ScriptPropertyType::InputAction &&
        definition.type != ScriptPropertyType::Scene) {
        return false;
    }
    auto stored =
        std::ranges::find(behavior.properties, definition.name, &ScriptPropertyValue::name);
    if (definition.type == ScriptPropertyType::AnimationClip) {
        ...
        const std::string preview = value.empty()          ? "None"
                                    : resolvedName.empty() ? value
                                                           : resolvedName;
        const auto acceptsAction = [&definition](const InputActionBinding& binding) {
        ...
        if (ImGui::BeginCombo(definition.name.c_str(), preview.c_str())) {
            const auto assignAction = [&](const std::string& action) {
                ...
                *stored = {};
                ...
                RecordImmediateEdit("Modify Script Input Action Property", propertyBefore,
                                    selectionBefore);
        ...
        if (ImGui::BeginCombo(definition.name.c_str(), preview.c_str())) {
            const auto assignScene = [&](const std::string& scene) {
                ...
                *stored = {};
                ...
                RecordImmediateEdit("Modify Script Scene Property", propertyBefore,
                                    selectionBefore);
